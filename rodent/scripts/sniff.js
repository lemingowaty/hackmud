function( C, A ) { //rodent.sniff{ channels:#s.chats.channels , join:#s.chats.join , leave:#s.chats.leave , level:4 , sector:0 }
// SETUP {
   let channel, list
   ,  B = #fs.scripts.lib()
   ,  {  sector , level
         , channels , join , leave } = A || {}
   ,  setSector = _ => {
         list = level.call()
         sector = list[ sector ]
      }
   ...
   level = ( _ => {
      switch ( level ) {
         default:
            return ( {
               call: q => (
                  #fs.scripts.fullsec( q )
               )
            } )
         case 3:
            return ( {
               call: q => (
                  #fs.scripts.highsec( q )
                  )
            } )
         case 2:
            return ( {
         ...
         case 0:
            return ( {
               call: q => (
                  #fs.scripts.nullsec( q )
               )
            } )
      }
   } )()
   let joined = !( channels.call().includes( sector ) ) ? 1 : 0
