function(ctx, arg)//{t:target_name.access,d:""}
{
	...
	var t = arg.t || 0;
	var q = {};
	function col(lt){ //Color lock Brute Forcer (Algorithm TBD)
		var ep = "c0"+lt;
		var x = 0;
		for (let i in ws.clr){
			q[ep] = ws.clr[i];
	...
	if(y===-1){
		t.li = glt(t.rs[0]);
		t.ln = lck[t.li]} 	
	t["?"] = function(){if(t.lkrs("ction") >= 0 ||t.lkrs("eached") >= 0||t.li<0){return true}return false}
	t.term = t["?"]();
	return }
	// - - - - - -
	// Main loop
	...
	while (t.term===false||t.term===undefined){
		...
		if (t.li===-1){break}
		if (t.li<=2) {ezc(t.ln)} else {col(t.ln)}
		
	}
	// End Of Main Loop
		t.log = b.get_log()
	if (arg.d!==undefined){return t}else{ return {ok:true} }
}