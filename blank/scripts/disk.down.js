function(C,A){
// DB handler //
	if(!A||!A.cmd||A.pwd!="carrots") {
			if(!A) return "blank.disk{}"
			else if(C.caller!="blank" && A.pwd!="carrots") throw Error("Unauthorized Entry. \n Contact Admin")
			if(!A.cmd)return "\t|`D No Command` |\n\t{ cmd : \"help\" }";
	}
	


 var que = A.que || {},//DB Query object
